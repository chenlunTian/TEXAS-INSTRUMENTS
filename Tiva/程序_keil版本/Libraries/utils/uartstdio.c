...
/*******************************************************************************
** 函数名称: UARTStdioConfig
** 功能描述: 配置 UART 控制台
** 参数说明: ui32PortNum是用于串行控制台的 UART 端口 
**			 ui32Baud是将 UART 配置为使用的波特率
**			 ui32SrcClock是 UART 的源时钟频率(标注：launchpad上为16mhz ，则为16000000)
** 返回说明: None
** 描    述: 该函数将配置指定的串行端口，以作为串行控制台。串口参数被设置为由ui32Baud 
**           参数指定的波特率，使用8位数据 ，没有奇偶校验位 ， 1 停止位。在使用任何
** 					 其他 UART 控制台函数 :UARTprintf() 或 UARTgets() 之前，必须调用此函数。 
** 					 该函数假设调用者之前已经将相关的 UART 引脚 配置为 UART ，而不是 GPIOs 。
** 创建人员: CLTian
** 创建日期: 2019-07-22
**------------------------------------------------------------------------------
********************************************************************************/
